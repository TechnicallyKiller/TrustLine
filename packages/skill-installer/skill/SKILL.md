...
uncollateralized USDC credit** on TrustLine (Stellar/Soroban). The agent holds
its own Stellar key, so the whole lifecycle is agent-driven — every on-chain
write is signed by the agent, never by TrustLine.
...
|--|--|--|--|
| **JavaScript / TypeScript** | `@trustline-agents/agent-sdk` | `import { TrustLineAgent }` | camelCase, options object, `async`/`await` |
| **Python** | `trustline-agent-sdk` | `from trustline import TrustLineAgent` | snake_case, keyword args, synchronous |
...
const tl = new TrustLineAgent(secret, {
  apiBaseUrl: "https://fianza-5m68.onrender.com", // the TrustLine backend
...
tl = TrustLineAgent(
    ...
    api_base_url="https://fianza-5m68.onrender.com",  # the TrustLine backend
...
SDK reimplements the `@x402/stellar` exact scheme (`trustline/x402.py`, and the
TS `x402` helper).
...
## Errors (all extend/subclass `TrustLineError`)

**JavaScript / TypeScript**
```ts
import { TrustLineError, ValidationError, ApiError, TxError, MaxDrawExceededError }
  from "@trustline-agents/agent-sdk";
```
