#!/usr/bin/env node
// TrustLine agent-sdk quickstart — a genuine, runnable end-to-end walkthrough.
//
// Generates a fresh Stellar testnet keypair, funds it (Friendbot XLM + a
// TrustLine faucet USDC drip), then runs the real credit loop:
...
async function main() {
  ...
  const tl = new TrustLineAgent(keypair.secret(), { apiBaseUrl: API });
