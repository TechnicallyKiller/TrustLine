...
<Note>
Always cross-check against the live `GET /config` on the backend (`https://fianza-5m68.onrender.com/config`) before sending a transaction — it's the source of truth the frontend and SDK actually resolve against.
</Note>
